%%*************************************************
%   Simulation of a 1-tissue compartment model with endogenous NT
%   competition (ntPET)
%   
%   Usage: model1TC
%
...
%        ode_1tc to slove ODE
%        get_properties.m define kinetic parameters for radiotracers
%        ntInput.m to generate endogenous neurotransmitter pulse
%
%   Author: Tom Morin
%   Adapted From: 'model2TC_DA.m' by Hsiao-Ying Monica Wey (Aug, 2013)
%   Date: June 2015
%%*************************************************

function plot_data = model1TC_DA(tracer_props, input_func, t, competition, noise, chal_times, amplitude, handles)
...
if strcmp(model, cellstr('1-Tissue Irreversible'))
    if ~isnan(k)
        k(2) = 0; 
    end
    if ~isnan(kp) 
...
if get(handles.competition,'Value') ~= 2
    [T,C] = ode23s(@(t,c) ode_1tc_da(t,c,tCa,Ca,t_EN,NT),t,[0,0]);   % with dynamic term;
% Solve the ODE in segments if Challenge is 'Artificial (Use k1 & k2)'
else
    k_34s = get_k_34s(handles);
    num_chals = length(chal_times);
    time_interval = [0:0.5:chal_times(1)-0.5];
    [T,C] = ode23s(@(t,c) ode_1tc_da(t,c,tCa,Ca,t_EN,NT),time_interval,[0,0]);
    for m=1:num_chals
        if m==num_chals
        ...
        k = [k_34s(1,m), k_34s(2,m), 0, 0];
        if strcmp(model, '1-Tissue Irreversible')
            k(2) = 0;
        end
        [T_temp,C_temp] = ode23s(@(t,c) ode_1tc_da(t,c,tCa,Ca,t_EN,NT),time_interval,[C(end,1),C(end,2)]);
        T = cat(1,T,T_temp);
        C = cat(1,C,C_temp);
    end
    T = T';
...
% Cf_n = add_noise(t,C(:,1)',noise);
% Cb_n = add_noise(t,C(:,2)',noise);
% Ce_n = add_noise(t,C(:,3)',noise);
Ct_n = noiseModel(t,C(:,1)',handles,noise);
Ce_n = noiseModel(t,C(:,2)',handles,noise);
